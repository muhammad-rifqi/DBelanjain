...
function selesai_belanja(){
	if (isset($_POST['submit'])){
		...
		if ($this->session->idp!=''){
			...
			foreach ($reseller_cek->result_array() as $row) {
				...
				$no = 1;
				$keranjang = $this->db->query("SELECT a.*, b.id_reseller, b.fee_produk FROM `rb_penjualan_temp` a JOIN rb_produk b ON a.id_produk=b.id_produk where session='".$this->session->idp."' AND b.id_reseller='$row[id_reseller]'");
				foreach ($keranjang->result_array() as $row) {
	...
	public function detail(){
		...
		if (isset($_POST['submit_rating'])){
			...
			$this->load->view('phpmu-marketrancak/reseller/view_produk_detail',$data);		
		}
	}
